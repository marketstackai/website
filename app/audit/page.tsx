...
function AuditPageContent() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      const redirectUrl = `/audit/start${queryString ? `?${queryString}` : ""}`;
  ...
  const submitToGHL = async (
    ...
    const auditRecord: GHLAuditRecord = {
      ...
      recommended:
        recommendedMap[computed.recommendedPackage] ?? "foundation_kit",
    };
  ...
  const handleFinalSubmit = async (
    e: React.FormEvent,
    overrides?: { contact: ContactFormData; quiz: typeof quizData },
  ) => {
    e.preventDefault();
    ...
    const recordId = await submitToGHL(
      effectiveContact,
      effectiveQuiz,
      computed,
    );
  ...
  const devAutofill = () => {
    const fills: Record<number, Partial<typeof quizData>> = {
      ...
      9: {
        ...
        ai_detail:
          "We use ChatGPT for marketing copy and have a basic 'thank you' email, but no automated qualification or scheduling.",
      },
      10: { urgency: "urgent" },
      11: {
        additional_notes:
          "Main focus is reducing response time for after-hours leads and automating appointment setting to free up our office staff.",
      },
    };
  ...
  const devAutoSubmit = async () => {
    ...
    const fullQuiz: typeof quizData = {
      ...
      ai_experience: "ai_some_results",
      ai_detail:
        "Using ChatGPT for some basic tasks but looking for a more integrated lead management solution that connects to our CRM.",
      urgency: "urgent",
      additional_notes:
        "We need a way to handle high lead volume more efficiently without hiring more office staff. Interested in lead recovery and automated SMS follow-ups.",
    };
    ...
    return (
      <div
        ...
        onClick={() => {
          ...
          if (autoNext) setTimeout(nextStep, 350);
        }}
        className={cn(
          "border-border hover:border-brand flex cursor-pointer items-center justify-between rounded-xl border p-4 transition-all",
          isSelected &&
            "border-brand bg-brand/10 shadow-[0_0_20px_rgba(var(--brand-rgb),0.1)]",
        )}
      >
        ...
        >
          {isSelected && (
            <div className="bg-brand animate-in fade-in zoom-in size-2.5 rounded-full duration-300" />
          )}
        </div>
      </div>
    );
  ...
  const renderMultiselectOption = (value: string, label: string) => {
    ...
    return (
      <div
        ...
        onClick={() => {
          const next = isSelected
            ? quizData.biggest_challenges.filter((v) => v !== value)
            : [...quizData.biggest_challenges, value];
      ...
      >
        ...
        <div
          className={cn(
            "ml-4 flex size-5 shrink-0 items-center justify-center rounded border-2 transition-colors",
            isSelected ? "border-brand bg-brand" : "border-muted-foreground/30",
          )}
        >
          {isSelected && <Check className="text-background size-3.5" />}
        </div>
      </div>
    );
  };
  ...
  const progress =
    step !== null && step >= 1 && step <= 12 ? (step / TOTAL_STEPS) * 100 : 0;
    ...
    <>
      ...
      {step >= 2 && step <= 11 && (
        <div className="bg-background text-foreground relative flex min-h-screen flex-col overflow-hidden">
          <div className="bg-muted relative z-50 h-1.5 w-full shrink-0">
            <div
              className="bg-brand h-full transition-all duration-300 ease-out"
              style={{ width: `${progress}%` }}
            />
          </div>
          <div className="text-muted-foreground pointer-events-none absolute top-1.5 left-0 z-50 flex w-full items-center justify-between p-2 sm:p-8">
            <Button
              variant="ghost"
              size="sm"
              onClick={prevStep}
            ...
            <span className="pt-1.5 pr-2 text-sm font-medium sm:pt-0">
              Step {step} of {TOTAL_STEPS}
            </span>
          </div>
          <div className="animate-appear flex flex-1 flex-col items-center overflow-y-auto px-4 pt-16 sm:justify-center sm:px-6 sm:pt-0">
            <form
              name="audit"
              onSubmit={handleAuditFormSubmit}
              className="mb-12 w-full max-w-lg sm:mb-0"
            >
              {step === 2 && (
                <div className="space-y-6">
                  <h2 className="mb-6 text-2xl font-semibold text-balance">
                    What type of business do you operate?
                  </h2>
                  {renderOption(
                    "home_services",
                    "Home Services (HVAC, Roofing, etc)",
                    "industry",
                  )}
                  {renderOption(
                    "real_estate",
                    "Real Estate (Brokerage, Investor, etc)",
                    "industry",
                  )}
                  {renderOption(
                    "professional_service",
                    "Professional Service (Attorney, CPA, etc)",
                    "industry",
                  )}
                  ...
                  {quizData.industry === "other" && (
                    <input
                      type="text"
                      className="bg-background focus:border-brand w-full rounded-lg border px-4 py-2.5 outline-hidden"
                      placeholder="e.g. Solar Installation"
                      value={quizData.industry_other}
                      onChange={(e) =>
                        setQuizData({
                          ...quizData,
                          industry_other: e.target.value,
                        })
                      }
                      autoFocus
                    />
              ...
              {step === 3 && (
                <div className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <p className="text-muted-foreground text-sm text-balance opacity-80">
                      Includes you, full-time staff, and long-term contractors.
                    </p>
                  </div>
                  <div className="border-border hover:border-brand/40 focus-within:border-brand flex w-full items-center justify-center overflow-hidden rounded-2xl border bg-black p-8 shadow-xl transition-all">
                    <input
                      type="number"
                      className="w-full border-none bg-transparent text-center text-5xl font-bold text-white tabular-nums outline-hidden focus:ring-0"
                      placeholder="5"
                      value={quizData.team_size}
                      onChange={(e) =>
                        setQuizData({ ...quizData, team_size: e.target.value })
                      }
                      autoFocus
                    />
                  </div>
                  <Button
                    ...
                    onClick={nextStep}
                    disabled={
                      !quizData.team_size || parseInt(quizData.team_size) < 1
                    }
                  >
                    Next
                  </Button>
              ...
              {step === 4 && (
                <div className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <p className="text-muted-foreground text-sm text-balance opacity-80">
                      This helps us estimate your lead volume accurately.
                    </p>
                  </div>
                  {quizData.monthly_revenue !== "undisclosed" ? (
                    <div className="flex flex-col items-stretch">
                      <div className="border-border focus-within:border-brand hover:border-brand/40 group flex w-full items-center justify-center overflow-hidden rounded-2xl border bg-black p-8 shadow-lg transition-all">
                        <div className="flex items-center">
                          <span className="pr-2 text-4xl font-bold text-white select-none">
                            $
                          </span>
                          <input
                            type="number"
                            className="border-none bg-transparent p-0 text-5xl font-bold text-white tabular-nums outline-hidden placeholder:text-white/20 focus:ring-0"
                            placeholder="175000"
                            style={{
                              width: `${Math.max(quizData.monthly_revenue?.length || 0, 6)}ch`,
                            }}
                      ...
                      <p className="text-muted-foreground/60 mt-4 text-center text-[10px] font-bold tracking-[0.2em] uppercase">
                        USD per month
                      </p>
                    </div>
                  ) : (
                    <div className="border-border text-muted-foreground bg-muted/20 rounded-xl border p-8 text-center">
                      <p className="text-foreground text-lg font-medium">
                        Revenue not disclosed
                      </p>
                      <p className="mt-1 mb-4 text-sm">
                        We&apos;ll estimate based on your team size.
                      </p>
                      <button
                        type="button"
                        onClick={() =>
                          setQuizData({ ...quizData, monthly_revenue: "" })
                        }
                        className="text-brand cursor-pointer text-xs font-semibold underline-offset-4 hover:underline"
                      >
                  ...
                  <Button
                    ...
                    onClick={nextStep}
                    disabled={
                      !quizData.monthly_revenue ||
                      (quizData.monthly_revenue !== "undisclosed" &&
                        (parseFloat(quizData.monthly_revenue) || 0) < 1)
                    }
                  >
                    Next
                  </Button>
              ...
              {step === 5 && (
                <div className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <p className="text-muted-foreground text-sm text-balance opacity-80">
                      What does a typical customer pay you per job or
                      engagement?
                    </p>
                  </div>
                  {quizData.avg_job_value !== "undisclosed" ? (
                      ...
                      <div className="border-border focus-within:border-brand hover:border-brand/40 group flex w-full items-center justify-center overflow-hidden rounded-2xl border bg-black p-8 shadow-lg transition-all">
                        <div className="flex items-center">
                          ...
                          <input
                            ...
                            placeholder="6000"
                            style={{
                              width: `${Math.max(quizData.avg_job_value?.length || 0, 4)}ch`,
                            }}
                            value={quizData.avg_job_value}
                            onChange={(e) =>
                              setQuizData({
                                ...quizData,
                                avg_job_value: e.target.value,
                              })
                            }
                            autoFocus
                            min={1}
                    ...
                    <div className="border-border text-muted-foreground bg-muted/20 rounded-xl border p-8 text-center">
                      <p className="text-foreground text-lg font-medium">
                        Job value not disclosed
                      </p>
                      <p className="mt-1 mb-4 text-sm">
                        We&apos;ll estimate based on your industry.
                      </p>
                      <button
                        type="button"
                        onClick={() =>
                          setQuizData({ ...quizData, avg_job_value: "" })
                        }
                        className="text-brand cursor-pointer text-xs font-semibold underline-offset-4 hover:underline"
                      >
                  ...
                  <Button
                    ...
                    onClick={nextStep}
                    disabled={
                      !quizData.avg_job_value ||
                      (quizData.avg_job_value !== "undisclosed" &&
                        parseFloat(quizData.avg_job_value) < 1)
                    }
                  >
                    Next
                  </Button>
              ...
              {step === 6 && (
                <div className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <p className="text-muted-foreground text-sm text-balance opacity-80">
                      Calls, form fills, messages — any new inquiry counts.
                    </p>
                  </div>
                  {quizData.monthly_leads !== "not_sure" ? (
                    <div className="flex flex-col items-stretch">
                      <div className="border-border focus-within:border-brand hover:border-brand/40 group flex w-full items-center justify-center overflow-hidden rounded-2xl border bg-black p-8 shadow-lg transition-all">
                        <input
                          type="number"
                          className="w-full border-none bg-transparent p-0 text-center text-5xl font-bold text-white tabular-nums outline-hidden placeholder:text-white/20 focus:ring-0"
                          placeholder="50"
                          value={quizData.monthly_leads}
                          onChange={(e) =>
                            setQuizData({
                              ...quizData,
                              monthly_leads: e.target.value,
                            })
                          }
                          autoFocus
                          min={0}
                    ...
                    <div className="border-border text-muted-foreground bg-muted/20 rounded-xl border p-8 text-center">
                      <p className="text-foreground text-lg font-medium">
                        Lead volume not provided
                      </p>
                      <p className="mt-1 mb-4 text-sm">
                        We&apos;ll estimate based on your revenue and job value.
                      </p>
                      <button
                        type="button"
                        onClick={() =>
                          setQuizData({ ...quizData, monthly_leads: "" })
                        }
                        className="text-brand cursor-pointer text-xs font-semibold underline-offset-4 hover:underline"
                      >
                  ...
                  <Button
                    ...
                    onClick={nextStep}
                    disabled={
                      !quizData.monthly_leads ||
                      (quizData.monthly_leads !== "not_sure" &&
                        (parseFloat(quizData.monthly_leads) || 0) < 1)
                    }
                  >
                    Next
                  </Button>
              ...
              {step === 7 && (
                <div className="space-y-4">
                  <div className="mb-6 space-y-2 text-balance">
                  ...
                  {renderMultiselectOption(
                    "missed_leads",
                    "Missing calls from leads and customers",
                  )}
                  {renderMultiselectOption(
                    "pipeline_stuck",
                    "Inconsistent sales",
                  ...
                  {renderMultiselectOption(
                    "tool_disconnect",
                    "Apps and tools lack integration",
                  )}
                  {renderMultiselectOption(
                    "unsure_ai",
                    "Not sure how AI could actually help",
                  )}
                  <Button
                    type="button"
                    className="mt-6 w-full"
                    size="lg"
                    onClick={nextStep}
                    disabled={quizData.biggest_challenges.length === 0}
                  >
                    Next
                  </Button>
                </div>
                  ...
                  {quizData.ai_experience === "no_ai" && (
                    <div className="animate-in fade-in slide-in-from-top-2 mt-4 space-y-3 duration-300">
                      <label className="text-muted-foreground pl-1 text-sm font-medium">
                        What&apos;s holding you back from trying AI?
                      </label>
                      <textarea
                        className="bg-background focus:border-brand min-h-[100px] w-full rounded-lg border px-4 py-2.5 outline-hidden"
                        placeholder="e.g. Not sure where to start, concerned about cost, don't trust the technology..."
                        value={quizData.ai_detail}
                        onChange={(e) =>
                          setQuizData({
                            ...quizData,
                            ai_detail: e.target.value,
                          })
                        }
                      />
                    </div>
                  )}
                  {(quizData.ai_experience === "ai_some_results" ||
                    quizData.ai_experience === "ai_advanced") && (
                    <div className="animate-in fade-in slide-in-from-top-2 mt-4 space-y-3 duration-300">
                      <label className="text-muted-foreground pl-1 text-sm font-medium">
                      ...
                      <textarea
                        className="bg-background focus:border-brand min-h-[100px] w-full rounded-lg border px-4 py-2.5 outline-hidden"
                        placeholder="e.g. Using ChatGPT for content, Zapier for lead alerts, or an AI chatbot on our site..."
                        value={quizData.ai_detail}
                        onChange={(e) =>
                          setQuizData({
                            ...quizData,
              ...
              {step === 10 && (
                <div className="space-y-6 text-balance">
                  <h2 className="mb-6 text-2xl font-semibold">
                    How soon are you looking to implement new systems?
                  </h2>
              ...
              {step === 11 && (
                <div className="space-y-6 text-balance">
                  ...
                  <textarea
                    ...
                    className="bg-background focus:border-brand min-h-[120px] w-full rounded-lg border px-4 py-2.5"
                    placeholder="e.g. We're looking to scale to $200k/mo and are interested in AI lead qualification and automated appointment booking."
                    value={quizData.additional_notes}
                    onChange={(e) =>
                      setQuizData({
                        ...quizData,
                  ...
                  <div className="ghl-capture-only" aria-hidden="true">
                    ...
                    <input
                      ...
                      value={quizData.biggest_challenges.join(", ")}
                      className="ghl-capture-only"
                      tabIndex={-1}
                    />
                    ...
                    <input
                      ...
                      readOnly
                      name="urgency"
                      value={quizData.urgency}
                      className="ghl-capture-only"
                      tabIndex={-1}
                    />

                    {computedFields &&
                      Object.entries(computedFields).map(([k, v]) => {
                        const labelText = k
                          .split("_")
                        ...
                        return (
                          <React.Fragment key={k}>
                            <label className="ghl-capture-only">
                              {labelText}
                            </label>
                            <input
                              type="text"
        ...
        createPortal(
          <div className="fixed right-4 bottom-4 z-[9999] flex items-center gap-2">
            <Button
              ...
              className="bg-background/80 border-muted-foreground/20 flex items-center gap-2 opacity-80 shadow-xl backdrop-blur-md transition-all hover:opacity-100"
            >
              <input
                type="checkbox"
                checked={ghlEnabled}
            ...
            <Button
              ...
              className="bg-background/80 border-muted-foreground/20 opacity-80 shadow-xl backdrop-blur-md transition-all hover:opacity-100"
            >
              Clear Session
            </Button>
            <Button
