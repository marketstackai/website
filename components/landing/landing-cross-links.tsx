        ...
        <div className="flex flex-wrap justify-center gap-3 max-w-[640px] mx-auto">
          {links.slice(0, 3).map((link, i) => (
            <Link
              key={i}
              href={link.href}
              className="group flex items-center gap-2 rounded-full border border-border bg-background px-4 py-2 text-sm font-medium text-muted-foreground hover:border-brand/40 hover:text-foreground transition-colors"
            >
              {link.label}
